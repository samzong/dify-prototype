...
import type { Knowledge2Item, KnowledgeDetailTab } from './knowledge-2-data'
import { ActionToast, EmptyPanel, Panel, TaskRow } from './knowledge-2-panel'
...
export function KnowledgeOverviewView({
  ...
  item: Knowledge2Item
  onNavigate: (tab: KnowledgeDetailTab) => void
}) {
  ...
  const taskDestination = (title: string): KnowledgeDetailTab | undefined => {
  ...
  const blockerDestination = (title: string): KnowledgeDetailTab => {
    const lower = title.toLowerCase()
...
function RuntimeRiskBanner({
  ...
  const risks: { text: string; tab?: KnowledgeDetailTab }[] = [
  ...
  ].filter(Boolean) as { text: string; tab?: KnowledgeDetailTab }[]
